			...
			var fileNameRow = $('<tr>').append(
				$('<td>').attr('colspan', '8').append(
					$('<div>').addClass('form-group').append(
						$('<input>').addClass('form-control').attr({type: 'text', name: 'project_product_name', id: 'project_product_name', placeholder: '檔案名稱',maxlength: 14})
					)
				),
                    ...
                    Promise.all(rpcPromises).then(() => {
                        if (self.nop === false) {
                            ...
                            if (isDoubleBiao)
                                ...
                                totalPrice = quantity * (jiagongPrice + basePrice * total_units) + peijianPrice;
        ...
        onSearchInput: function() {
            ...
            if (keyword.length >= 1) {
                rpc.query({
                    ...
                    fields: ['id','name', 'mobile', 'phone', 'email', 'street','customclass_id','custom_init_name'], 
                    limit: 10,
