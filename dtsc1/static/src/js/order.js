odoo.define('dtsc.order', function (require) {
    ...
    function getCurrentUserInfo() {
        ajax.jsonRpc("/my/user_info", 'call', {})
        .then(function (data) {
    ...
    return tableContent;
		...
		initProductVariantChangeListener: function(selectedProductId) {//產品變體選擇事件
			...
			$(document).on('change', '.product_variant', async function() {
				...
				if(outerSelf.checkAllSelected())
				{
					...
					var newLabel10 = $('<label>').addClass('custom-file-label').attr('for', 'custom_file').text('限制500M，超過請聯繫管理員');
                ...
                if (selectedProductId) {
                    try {
                        ...
                        for (let line of attributeLines) {
                            ...
                            let filteredValues = [];//過濾is_visible_on_order
                            ...
                            debugLog("Filtered attributeValues:", filteredValues);
                            ////////////////////////////
		...
		showCustomerDetails: function(selectedCustomer) { 
			...
			this.fetchSaleProductCategories(selectedCustomer.customclass_id).then(categories => {
        ...
        onSearchInput: function() {
            ...
            if (keyword.length >= 1) {
                rpc.query({
                    model: 'res.partner',
                    method: 'search_read',
                    domain: ['|', '|', ['name', 'ilike', keyword], ['mobile', 'ilike', keyword], ['phone', 'ilike', keyword]],
                    fields: ['id','name', 'mobile', 'phone', 'email', 'contact_address_complete','customclass_id'], 
                    limit: 10,
                    ...
                    $.each(customers, function(i, customer) {						
                        ...
                        listItem.click(function() {
							...
							self.customer_class_id = customer.customclass_id//客户分类id
                            $('.customer-detail').remove(); 
