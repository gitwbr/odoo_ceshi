...
class LotMpr(models.Model):
    ...
    def _compute_lot_stock_num(self):
        ...
        for record in self:
            if record.product_lot.id:
                ...
                quant = self.env["stock.quant"].search([('product_id' , "=" , record.product_id.id),("lot_id" ,"=" , record.product_lot.id),("location_id" ,"=" ,8)],limit=1) #這裡出現的負數我用company_id隱藏，未來要修正
    ...
    def confirm_btn(self):
        quant = self.env["stock.quant"].search([('product_id' , "=" , self.product_id.id),("lot_id" ,"=" , self.product_lot.id),("location_id" ,"=" ,8)],limit=1) #這裡出現的負數我用company_id隱藏，未來要修正
    ...
    def ok_btn(self):
        ...
        for record in self.lotmprline_id:
            ...
            if (now_stock_p - record.yujixiaohao) < 0 :
                ...
                quant = self.env["stock.quant"].search([('product_id' , "=" , self.product_id.id),("lot_id" ,"=" , self.product_lot.id),("location_id" ,"=" ,8)],limit=1) 
                print(quant)
                if quant:
...
class LotMprLine(models.Model):
    ...
    def _compute_shengyu(self):
        tmp = self.lotmpr_id.total_size * float(self.lotmpr_id.lot_stock_num)
