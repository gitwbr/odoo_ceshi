    ...
    def action_confirm(self):
        ...
        for record in records:  
            ...
            for line in record.product_ids:
                ...
                line.delivery_order = new_name + "-" + str(sequence_number)                
...
class Checkout(models.Model):
    ...
    # customer_class_id = fields.Many2one(related='customer_id.customclass_id' )
    project_name = fields.Char(string='案件摘要')
    delivery_carrier = fields.Char(string='運送方式')
    ...
    create_id = fields.Many2one('res.users',string="開單人員", default=lambda self: self.env.user)
    comment = fields.Char(string="訂單備註")
    # comment_customer = fields.Char(string="客戶備註")
    comment_factory = fields.Char(string="廠區備註" ,inverse="_inverse_comment_factory",compute="_compute_comment" ,store=True)
    ...
    recheck_user = fields.Many2many('res.users',string="重製相關人員")
    ...
    #wbr订单进度wbr订单进度
    order_content = fields.Char(string="訂單內容", compute="_compute_order_content")

    @api.depends('product_ids.product_id')
    def _compute_order_content(self):
        for record in self:
            product_names = record.product_ids.mapped('product_id.name')
            record.order_content = ' + '.join(product_names)
    #wbr订单进度
    ...
    def _compute_search_line_project_product_name(self):
        for record in self:
            ...
            result = ', '.join([
                ...
                combined_machine_id_names,self.project_name or '',self.name or ''
            ])
    ...
    def onchange_customer_id(self):
        # print(self.customer_id.id)
        # print(self.customer_id.customclass_id)
            ...
            if make_in_flag == 1:
                ...
                if obj:
                    if obj.install_state != "stock_in":
                        raise UserError('委內工單還未完成！')
                else:
                    raise UserError('委內工單還未生成！')
    ...
    def in_check(self):
        ...
        if self.name and self.name[0] == 'E':
            install_name = self.name#.replace("A","B")
            is_install_id = self.env['dtsc.makein'].search([('name', '=',install_name+"-E")],limit=1)
            if is_install_id:
                return
        # install_name = self.name.replace("A","B").replace("E","B")
        # if self.name and self.name[0] == 'E':
        ...
        for record in self.product_ids:
            if record.is_purchse == 'make_in':
                ...
                record.make_orderid = install_name + "-" + str(sequence_number)
                product_values_list.append((0,0,product_value))
                sequence_number += 1
    ...
    def deliveryorder(self):
        ...
        if not self.estimated_date:
            ...
            for record in self.product_ids:  
                ...
                record.delivery_order = new_name + "-" + str(sequence_number)  #记录该条数据的出货单编号
...
class CheckOutLine(models.Model):
    ...
    # make_type = fields.Many2many("後加工","")
    ...
    product_id = fields.Many2one("product.template",string='商品名稱' ,required=True,domain=['|', '&', ('sale_ok', '=', True), ('categ_id.name', '!=', "展示"), ('can_be_expensed', '=', True)]) 
    ...
    peijian_price = fields.Float("配件加價", compute='_compute_peijian_price', inverse="_inverse_peijian_price" ,default=0)
    single_units = fields.Float("單一才數" , compute='_compute_single_units')
    total_units = fields.Float("總才數" , compute='_compute_total_units', inverse="_inverse_total_units" ,default=0)
    units_price = fields.Float("單價", compute='_compute_units_price', inverse="_inverse_units_price" ,default=0 , store=True)
    ...
    total_make_price = fields.Float("加工金額", compute='_compute_total_make_price', inverse="_inverse_total_make_price" ,default=0)
    ...
    sequence = fields.Integer(string='項次', readonly=True, copy=False)
    ...
    def _compute_multi_chose_ids(self):
        ...
        for record in self:
            ...
            for after_record in record.aftermakepricelist_lines:
                ...
                tmp += after_record.aftermakepricelist_id.name.name +"*" + str(round(after_record.qty)) + " - "
                
            record.multi_chose_ids = tmp
    ...
    def _update_sequence(self):
        if self.checkout_product_id:
            children = self.env['dtsc.checkoutline'].search([('checkout_product_id', '=', self.checkout_product_id.id)], order='sequence')
    ...
    def create(self, vals):
        ...
        checkoutline_obj = super(CheckOutLine, self.with_context(context)).create(vals)
        checkoutline_obj._update_sequence()        
        return checkoutline_obj
    ...
    def _inverse_total_units(self):
        pass 
    def _inverse_peijian_price(self):
        pass
    ...
    def _compute_price(self): 
        ...
        for record in self:            
            ...
            if record.jijiamoshi in ["forcai", "merge"]:#以才計價
                ...
                record.price = record.units_price * record.quantity
        # print("end_compute_price")
    #配件以外的加工金额
    
    @api.depends('aftermakepricelist_lines',"aftermakepricelist_lines.qty",'product_atts',"units_price","total_units","jijiamoshi","mergecai","total_make_price")
    def _compute_total_make_price(self):   
        # print("_compute_total_make_price")
        for record in self:
            # print(record.manual_total_make_price_flag)
            # print(record.project_product_name)
            total_after_price = 0
            for after_record in record.aftermakepricelist_lines:
    ...
    #配件加价
    @api.depends('product_atts',"jijiamoshi")
    def _compute_peijian_price(self):
        ...
        for record in self:
            if record.jijiamoshi in ["forcai", "merge"]: #以才計價
                ...
                b = 0
                for attr_value in record.product_atts:
                    # print("bbb %d" %attr_value.id)
                    if isinstance(attr_value.id,int):
                        if attr_value.attribute_id.name == "配件":                            
                            a = self.env["dtsc.quotationproductattributeprice"].search([("quotation_id","=" ,quotation_id.id),("attribute_value_id","=" ,attr_value.id)],limit=1).price_jian
                            price_extra_record = self.env["product.attribute.value"].search([("id","=" ,attr_value.id)],limit=1)#加上浮動價格
                            price_extra = price_extra_record.price_extra if price_extra_record else 0
