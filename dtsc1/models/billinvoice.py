...
class Allowances(models.Model):
    ...
    def open_bill_btn(self):
        ...
        data["TaxAmount"] = round(TotalAmount * 0.05 + 0.1)
...
class BillInvoice(models.Model):
    ...
    def _compute_tax_value(self):
        for record in self:
            record.tax_value = round(record.sale_value * 0.05 + 0.1)
    ...
    def open_bill_btn(self):        
        ...
        TaxAmount = round((SalesAmount + 0.1) * 0.05)
...
class AccountMove(models.Model):
    ...
    tax_price = fields.Integer("稅額"  , compute="_compute_tax_price",store=True)
    total_price = fields.Integer("含稅總價" , compute="_compute_total_price",store=True)
    
    @api.depends("amount_untaxed_signed")
    def _compute_sale_price(self):
        for record in self:
            record.sale_price = int(round(record.amount_untaxed_signed+0.1))
    ...
    def _compute_tax_price(self):
        for record in self:
            if record.partner_id.custom_invoice_form in ["21","22"] or record.is_online == True:
                record.tax_price = int(round(record.sale_price * 0.05 + 0.1))
            else:
                record.tax_price = 0
